...
public class score0520 { // 類別名稱（class name）要與檔案名稱相同
  public static String getRank(int score) {// 判斷等第
  ...
  public static void main(String args[]) { // 主程式名稱、引數、小寫
    ...
    int[] chinese = new int[100];// 宣告整數陣列
    int[] english = new int[100];
    int[] math = new int[100];
    String[][] personal = new String[100][6];// [i][0]->國等第/[i][1]->英等第/[i][2]->數等第/[i][3]->總分/[i][4]->平均/[i][5]->平均等第
    float[][] calculate = new float[3][5];// [i][0]->max；[i][1]->min；[i][2]->ave；[i][3]->failed；[i][4]->pass
    int num = 0;// 宣告整數num計數
    while (true) {// 無窮迴圈
    ...
    System.out
        .println(
            "\n【全班國文成績】\n最高分：" + Math.round(calculate[0][0]) + "\t最低分：" + Math.round(calculate[0][1]) + "\t平均："
                + Math.round(calculate[0][2]) + "\t不及格人數："
                + Math.round(calculate[0][3]) + "\t及格人數：" + Math.round(calculate[0][4]));// 輸出國文細節
    System.out
        .println(
            "\n【全班英文成績】\n最高分：" + Math.round(calculate[1][0]) + "\t最低分：" + Math.round(calculate[1][1]) + "\t平均："
                + Math.round(calculate[1][2]) + "\t不及格人數："
                + Math.round(calculate[1][3]) + "\t及格人數：" + Math.round(calculate[1][4]));// 輸出英文細節
    System.out
        .println(
            "\n【全班數學成績】\n最高分：" + Math.round(calculate[2][0]) + "\t最低分：" + Math.round(calculate[2][1]) + "\t平均："
                + Math.round(calculate[2][2]) + "\t不及格人數："
                + Math.round(calculate[2][3]) + "\t及格人數：" + Math.round(calculate[2][4]));// 輸出數學細節
    while (true) {// 無窮迴圈
